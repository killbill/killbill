...
public class TestCatalogWithEffectiveDateForExistingSubscriptionsCustomConfigNonMonthlyPlans extends TestIntegrationBase {
    ...
    public void testMultiPhasePlanWithWeeklyBilling() throws Exception {
        ...
        //
        // liability-monthly-trial has a a 7-day TRIAL phase followed by an EVERGREEN phase
        //

        final Account account = createAccountWithNonOsgiPaymentMethod(getAccountData(5));
        busHandler.pushExpectedEvents(NextEvent.CREATE, NextEvent.BLOCK, NextEvent.INVOICE, NextEvent.INVOICE_PAYMENT, NextEvent.PAYMENT);
        final UUID entitlementId = entitlementApi.createBaseEntitlement(account.getId(), new DefaultEntitlementSpecifier(new PlanPhaseSpecifier("liability-monthly-with-trial-weekly"), null, null, UUID.randomUUID().toString(), null), "something", null, null, false, true, Collections.emptyList(), callContext);
    ...
    public void testMultiPhasePlanWithMonthlyBilling() throws Exception {
        ...
        //
        // liability-monthly-trial has a 1-month TRIAL phase followed by an EVERGREEN phase
        //

        final Account account = createAccountWithNonOsgiPaymentMethod(getAccountData(7));
        busHandler.pushExpectedEvents(NextEvent.CREATE, NextEvent.BLOCK, NextEvent.INVOICE, NextEvent.INVOICE_PAYMENT, NextEvent.PAYMENT);
        final UUID entitlementId = entitlementApi.createBaseEntitlement(account.getId(), new DefaultEntitlementSpecifier(new PlanPhaseSpecifier("liability-monthly-with-trial-monthly"), null, null, UUID.randomUUID().toString(), null), "something", null, null, false, true, Collections.emptyList(), callContext);
        final Entitlement bpEntitlement1 = entitlementApi.getEntitlementForId(entitlementId, false, callContext);
        assertListenerStatus();
        assertNotNull(bpEntitlement1);
