...
public class TestCatalogAnnualSubscription extends TestIntegrationBase {

    @Override
    protected KillbillConfigSource getConfigSource(final Map<String, String> extraProperties) {
        final Map<String, String> allExtraProperties = new HashMap<String, String>(extraProperties);
        allExtraProperties.put("org.killbill.catalog.uri", "catalogs/testCatalogAnnualSubscriptions");
        return super.getConfigSource(null, allExtraProperties);
    }

    @Test(groups = "slow", description = "https://github.com/killbill/killbill/issues/1881")
    public void testUpdateSubscriptionAnnualPlan() throws Exception {

        final LocalDate today = new LocalDate(2025, 1, 1);
        clock.setDay(today);

        final VersionedCatalog catalog = catalogUserApi.getCatalog("FAMILY", callContext);

        final Account account = createAccount(getAccountData(null));

        final DefaultEntitlement bpEntitlement =
                createBaseEntitlementAndCheckForCompletion(account.getId(), "externalKey1", "HOME20MB",
        ...
        // We have created an annual subscription that start on 2025-01-01 and end on 2026-01-01
        // Now we move the clock forward to 2025-02-16, and we update the plan.
        // I would expect the end date of the new plan to be set in 2026-01-01, with the cost calculated accordingly.
        //
        // 2025-02-16
        ...
        final DefaultEntitlement updateEntitlement =
                createBaseEntitlementAndCheckForCompletion(account.getId(), "externalKey1", "HOME100MB",
                                                           ProductCategory.BASE, BillingPeriod.ANNUAL,
                                                           NextEvent.CREATE, NextEvent.BLOCK, NextEvent.INVOICE);
