...
public class TestDefaultCustomFieldUserApi extends UtilTestSuiteWithEmbeddedDB {
    ...
    public void testCustomFieldNoId() throws Exception {
        ...
        customFieldUserApi.addCustomFields(List.of(customField), callContext);
    ...
    public void testCustomFieldBasic() throws Exception {
        ...
        customFieldUserApi.addCustomFields(List.of(customField1, customField2), callContext);
        assertListenerStatus();
        ...
        try {
            customFieldUserApi.addCustomFields(List.of(customField3, customField1), callContext);
        ...
        customFieldUserApi.addCustomFields(List.of(customField3), callContext);
        assertListenerStatus();
        ...
        eventsListener.pushExpectedEvents(NextEvent.CUSTOM_FIELD, NextEvent.CUSTOM_FIELD);
        customFieldUserApi.removeCustomFields(List.of(customField1, customField3), callContext);
        assertListenerStatus();
    ...
    public void testCustomFieldUpdate() throws Exception {
        ...
        eventsListener.pushExpectedEvents(NextEvent.CUSTOM_FIELD);
        customFieldUserApi.addCustomFields(List.of(customField1), callContext);
        assertListenerStatus();
        ...
        customFieldUserApi.updateCustomFields(List.of(update1), callContext);

        final List<CustomField> all = customFieldUserApi.getCustomFieldsForAccount(accountId, callContext);
        Assert.assertEquals(all.size(), 1);
        ...
        try {
            customFieldUserApi.updateCustomFields(List.of(new StringCustomField("gtqre", "value1", ObjectType.ACCOUNT, accountId, callContext.getCreatedDate())), callContext);
        ...
        try {
            customFieldUserApi.updateCustomFields(List.of(new StringCustomField(customField1.getId(), "wrongName", "value2", customField1.getObjectType(), customField1.getObjectId(), callContext.getCreatedDate())), callContext);
            Assert.fail("Updating custom field should fail");
        } catch (final CustomFieldApiException e) {
    ...
    public void testSaveCustomFieldWithAccountRecordId() throws Exception {
        ...
        eventsListener.pushExpectedEvent(NextEvent.CUSTOM_FIELD);
        customFieldUserApi.addCustomFields(List.of(newCustomField), callContext);
        assertListenerStatus();
