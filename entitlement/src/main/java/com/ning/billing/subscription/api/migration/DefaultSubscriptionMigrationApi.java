...
public class DefaultSubscriptionMigrationApi extends SubscriptionApiBase implements SubscriptionMigrationApi {

    private final MigrationPlanAligner migrationAligner;
    private final InternalCallContextFactory internalCallContextFactory;

    @Inject
    public DefaultSubscriptionMigrationApi(final MigrationPlanAligner migrationAligner,
    ...
    private List<SubscriptionEvent> toEvents(final SubscriptionData subscriptionData, final DateTime now, final DateTime ctd, final TimedMigration[] migrationEvents, final CallContext context) {


        if (ctd == null) {
            throw new SubscriptionError(String.format("Could not create migration billing event ctd = %s", ctd));
        }
            ...
            if (cur.getEventType() == EventType.PHASE) {
                ...
                switch (cur.getApiEventType()) {
                    ...
                    case CANCEL:
                        ...
                        break;
                    default:
                        throw new SubscriptionError(String.format("Unexpected type of api migration event %s", cur.getApiEventType()));
                }
            } else {
                throw new SubscriptionError(String.format("Unexpected type of migration event %s", cur.getEventType()));
            }
