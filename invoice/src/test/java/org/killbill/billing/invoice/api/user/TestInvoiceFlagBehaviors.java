...
public class TestInvoiceFlagBehaviors extends InvoiceTestSuiteWithEmbeddedDB {
    ...
    public void testWrittenOffInvoiceBeforeAccountCredit() throws Exception {
        ...
        final List<InvoiceItem> items = invoiceUserApi.insertExternalCharges(accountId, clock.getUTCToday(), List.of(new ExternalChargeInvoiceItem(UUID.randomUUID(), clock.getUTCNow(), null, accountId, null, null, null, null, BigDecimal.TEN, accountCurrency, null)), true, null, callContext);
        ...
        invoiceUserApi.insertCredits(accountId, clock.getUTCToday(), List.of(inputCredit), true, null, callContext);
    ...
    public void testWrittenOffInvoiceWithAccountCredit() throws Exception {
        ...
        // Create new invoice with one charge and expect account credit to be used
        final List<InvoiceItem> items = invoiceUserApi.insertExternalCharges(accountId, clock.getUTCToday(), List.of(new ExternalChargeInvoiceItem(UUID.randomUUID(), clock.getUTCNow(), null, accountId, null, null, null, null, new BigDecimal("13.5"), accountCurrency, null)), true, null, callContext);
        assertEquals(items.size(), 1);
    ...
    public void testMigratedInvoiceWithAccountCredit() throws Exception {
        ...
        final UUID invoiceId = invoiceUserApi.createMigrationInvoice(accountId, null, List.of(new FixedPriceInvoiceItem(UUID.randomUUID(), clock.getUTCNow(), null, accountId, null, null, null, "foo", "bar", null, null, null, BigDecimal.ONE, accountCurrency)), callContext);
    ...
    public void testDraftInvoiceWithAccountCredit() throws Exception {
        ...
        // Create new invoice with one charge and expect account credit to be used
        final List<InvoiceItem> items = invoiceUserApi.insertExternalCharges(accountId, clock.getUTCToday(), List.of(new ExternalChargeInvoiceItem(UUID.randomUUID(), clock.getUTCNow(), null, accountId, null, null, null, null, new BigDecimal("4.0"), accountCurrency, null)), false, null, callContext);
        assertEquals(items.size(), 1);
