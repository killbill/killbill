...
public class TestLoggingHelper {
    ...
    private static void injectLoggerIntoLoggerFactory(final Class loggingClass, final Logger logger) throws Exception {
        SimpleLoggerFactory simpleLoggerFactory = (SimpleLoggerFactory) StaticLoggerBinder.getSingleton().getLoggerFactory();
        Field loggerMapField = SimpleLoggerFactory.class.getDeclaredField("loggerMap");
        loggerMapField.setAccessible(true);
        Map loggerMap = (Map)loggerMapField.get(simpleLoggerFactory);
